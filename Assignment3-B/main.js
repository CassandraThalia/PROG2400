...
(() => {
    ...
    .then((json) => {
        ...
        //9 - Create a function called getEpisodeTallyBySeason that returns an object containing 
        //the season name and the total episodes as key:value pairs for each season
        console.log('--------------------------------');
        console.log(`Tally of episodes by season:`);
        console.log(getEpisodeTallyBySeason(json));


        //10 - Create a funtion called capitalizeTheFriends that transforms the episode JSON data by capitalizing the words 
        //Joey, Chandler, Monica, Rachel, Phoebe, and Ross in both the name and summary of the episodes.
        console.log('--------------------------------');
    ...
    const getGuntherCount = (json) => {
        let guntherCount = json['_embedded']['episodes'].filter((episode) => { 
            return episode.summary.indexOf('Gunther') > -1 
        });
        return guntherCount.length;
    };

    const getTotalRuntimeMinutes = (json) => {
        let sum = json['_embedded']['episodes'].reduce( (total, episode) => {
    ...
    const getTotalEpisodesInYear = (json) => {
        let epiCount =  json['_embedded']['episodes'].filter((episode) => {
            let date = new Date(episode.airdate)
            return date.getFullYear() >= 2000 && date.getFullYear() < 2001;
        });
        return epiCount.length;
    };

    const getFemaleCastMembers = (json) => {
        let femaleCastMembers = json['_embedded']['cast']
        .filter((castMember) => { 
            return castMember.person.gender === 'Female'
        })
        .map(castMember => castMember.person.name);
        return femaleCastMembers;
    };

    const getEpisodeTitles = (json, arg) => {
        let episodeTitles = json['_embedded']['episodes']
        .filter((episode) => { 
            return episode.summary.indexOf(arg) > -1
        })
        .map(episode => episode.name);
        return episodeTitles;
    };

    //Code Source: https://stackoverflow.com/questions/10008050/get-age-from-birthdate
    let getAge = (date) => {
        var today = new Date();
    ...
    const getCastMembersOver55 = (json) => {
        let castMembers = json['_embedded']['cast']
        .filter((castMember) => {
            return getAge(castMember.person.birthday) >= 55;
        })
        .map(castMember => castMember.person.name);
        return castMembers;
    };
    ...
    const getTotalRuntimeMinutesExcludingSeasonSix = (json) => {
        let sum = json['_embedded']['episodes']
        .filter( (episode) => {
    ...
    const getFirstFourSeasons = (json) => {
        let seasons = json['_embedded']['episodes']
        .filter( (episode) => {
    ...
    //Code source: https://vmarchesin.medium.com/using-array-prototype-reduce-in-objects-using-javascript-dfcdae538fc8
    //(Full disclosure -- I don't totally understand how this works, but it works!)
    const getEpisodeTallyBySeason = (json) => {
        let episodeTally = json['_embedded']['episodes']
        .reduce((map, episode) => (
            {
                ...map,
                [episode.season]: (map[episode.season] || 0) + 1, 
            }
        ), {});
        return episodeTally;
    };
    ...
    const capitalizeTheFriends = (json) => {
        let friendsNames = ['Joey', 'Chandler', 'Monica', 'Rachel', 'Phoebe', 'Ross'];
        let capitalizedFriends = json['_embedded']['episodes']
        .map ( (episode) => {
            let name = episode.name;
            let summary = episode.summary;
            friendsNames.forEach( (friend) => {
                if (episode.summary.indexOf(friend) > -1) {
                    summary = summary.replaceAll(friend, friend.toUpperCase());
                }
            })
            friendsNames.forEach( (friend) => {
                if (episode.name.indexOf(friend) > -1) {
                    name = name.replaceAll(friend, friend.toUpperCase());
                }
            })
            return {
                name: name,
